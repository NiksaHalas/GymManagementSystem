...
import {
  assignShiftToPayment,
  waivePaymentAttribution,
} from "@/lib/shifts/reconcile-actions";

interface PaymentReconcileActionsProps {
  paymentId: string;
  businessDate: string;
  shifts: ShiftOption[];
}
...
export function PaymentReconcileActions({
  ...
  async function handleAssign() {
    ...
    setPending(true);
    try {
      const result = await assignShiftToPayment(paymentId, shiftId);
  ...
  async function handleWaive() {
    setPending(true);
    try {
      const result = await waivePaymentAttribution(paymentId);
      if (result.error) toast.error(result.error);
      else {
