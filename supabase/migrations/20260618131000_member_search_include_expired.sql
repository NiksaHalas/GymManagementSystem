-- search_members: uključi 'istekla' članarine u prikaz statusa u listi članova.
-- Ista signatura (4 arg), isti grantovi / security invoker / search_path.
-- active_m CTE bira najrelevantniju članarinu po članu: prvo aktivna/pauzirana,
-- u suprotnom najnovija istekla (fallback), tako da lista prikazuje "Istekla"
-- umesto "Nema članarine" kad je članarina prebačena u istekla (promote_memberships).
...
as $$
  ...
  active_m as (
    ...
    where m.status in ('aktivna', 'pauzirana', 'istekla')
    order by
      m.member_id,
      case when m.status in ('aktivna', 'pauzirana') then 0 else 1 end,
      m.created_at desc
  ),
  base as (
    select me.*
