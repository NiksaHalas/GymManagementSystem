-- create_checkin(): no expired-session override while a time-based membership covers the visit.
--
-- The solo override branch (p_allow_expired_override) deducted a session from an
-- expired session-based Otvoreni package whenever the normal solo path had not
-- deducted one. That was also true for a member with an ACTIVE, in-date time-based
-- membership (Otvoreni 30/1, Kardio 30/1), which already covers the arrival, so
-- confirming the dialog burned a session from an old package for nothing
-- (PRD §3.4: time-based memberships deduct nothing; the override is for using
-- remaining sessions after expiry). The check-in dialog no longer offers it in that
-- case; this makes the RPC ignore the flag there too.
--
-- Trainer sessions are unchanged: a time-based Otvoreni membership does not cover a
-- trainer session (PRD §3.5), so the expired same-category trainer package override
-- still applies. Signature unchanged — create or replace, no type regen.
...
begin
  ...
  then
    ...
    v_solo_deducted := true;
        ...
        -- An active, in-date time-based membership already covers a solo arrival.
        and not (v_has_membership
                 and coalesce(v_mt.is_time_based, false)
                 and not v_active_expired)
  then
    select m.* into v_ovr
    from membership m
