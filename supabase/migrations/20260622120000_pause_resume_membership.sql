-- Pause / resume membership + create_checkin paused branch (PRD §3.4).
--
-- Adds pause_membership / resume_membership RPCs (GYM03 / GYM04).
-- Fixes create_checkin: a paused membership is frozen — record the arrival but
-- no session deduction, no reserved_session, no first-visit activation, and
-- checkin.membership_id = NULL.

-- ---------------------------------------------------------------------------
-- pause_membership(p_membership_id uuid) → void
-- ---------------------------------------------------------------------------
create or replace function pause_membership(p_membership_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  if not found then
    raise exception 'Članarina nije pronađena.' using errcode = 'GYM03';
  end if;
  ...
  if v_archived then
    raise exception 'Član je arhiviran.' using errcode = 'GYM03';
  end if;

  if v_row.status <> 'aktivna' then
    raise exception 'Članarina nije aktivna.' using errcode = 'GYM03';
  end if;

  if v_row.start_date is null then
    raise exception 'Članarina još nije aktivirana (prvi dolazak).' using errcode = 'GYM03';
  end if;
...
revoke execute on function pause_membership(uuid) from public, anon;
grant  execute on function pause_membership(uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- resume_membership(p_membership_id uuid) → void
-- ---------------------------------------------------------------------------
...
begin
  ...
  if not found then
    raise exception 'Članarina nije pronađena.' using errcode = 'GYM04';
  end if;
  ...
  if v_archived then
    raise exception 'Član je arhiviran.' using errcode = 'GYM04';
  end if;

  if v_row.status <> 'pauzirana' or v_row.paused_at is null then
    raise exception 'Članarina nije pauzirana.' using errcode = 'GYM04';
  end if;

  v_days := business_today() - (v_row.paused_at at time zone 'Europe/Belgrade')::date;
  if v_days < 0 then
    v_days := 0;
  end if;
  ...
  set status = 'aktivna',
      ...
      end_date = case when end_date is not null then end_date + v_days else end_date end,
      updated_by = v_staff_id,
      updated_at = now()
  where id = p_membership_id;
end;
...
revoke execute on function resume_membership(uuid) from public, anon;
grant  execute on function resume_membership(uuid) to authenticated;

-- ---------------------------------------------------------------------------
-- create_checkin — paused branch (signature unchanged)
-- Bazirano na 20260619140000; dodaje v_is_paused guard za membership link,
-- trainer side-effects i first-visit aktivaciju.
-- ---------------------------------------------------------------------------
