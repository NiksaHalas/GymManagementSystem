...
- **Solo session-based Otvoreni** (v1.22): `not p_with_trainer`, active membership with `training_category.code='otvoreni'` and `membership_type.is_time_based=false`, `sessions_left > 0` → decrement `membership.sessions_left`, set `decremented_session = true`; no `session_log`. `checkin.membership_id` = that membership. At 0 sessions → record check-in only (no deduction, no debt).
